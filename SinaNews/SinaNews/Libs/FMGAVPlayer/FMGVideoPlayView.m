...
{
    // 1.更新时间
    self.timeLabel.text = [self stringWithCurrentTime:CMTimeGetSeconds(self.player.currentItem.duration) - CMTimeGetSeconds(self.player.currentTime)];
    self.leftTimeLabel.text = [self stringWithCurrentTime:CMTimeGetSeconds(self.player.currentTime)];
...
- (IBAction)sliderValueChange {
    ...
    self.timeLabel.text = [self stringWithCurrentTime:duration - currentTime];
...
- (NSString *)stringWithCurrentTime:(NSTimeInterval)time
{
    ...
    NSString *timeString = [NSString stringWithFormat:@"%02ld:%02ld", dMin, dSec];
